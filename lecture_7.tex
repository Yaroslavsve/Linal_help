...
\begin{document}
  ...
  \begin{proof}
    ...
    Для первого случая: $\overline{e_j} = e_j$ + $U$, для $\overline{m+1, n}$, является базисом в фактор-пространстве $overline{V}$ = $V|\begin{matrix}
      \null \\ U
    \end{matrix} \overline{\phi}(\bar{e_j})$ = $\phi(e_j) = \overline{\sum \limits_{i=1}^m a_{ij} e_i + \sum \limits_{k=m+1}^n a_{kj} e_k} = \sum \limits_{k=m+1}^n e_{kj} \overline{e_k}$ (так как первая сумма $\in U$ )
    $\Longrightarrow C = \begin{pmatrix}
      a_{m+1, m+1} & \cdots & a_{m+1, n}\\
      \vdots\\
      a_{n, m+1} & \cdots & a_{nn}
    \end{pmatrix}$ - матрица оператора $\overline{\phi}$.
  \end{proof}
  \begin{remark}
